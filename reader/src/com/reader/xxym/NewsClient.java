...
public abstract class NewsClient {
	...
	public NewsClient(Activity activity, Integer currentKindID) {
		...
		downloader = new Downloader(context) {
			...
			protected void onPostExecute(String result) {
				...
				Pattern p = Pattern
						.compile(
								"<a href=\"(http://soul.cn.yahoo.com/ypen/\\d+/\\d+.html)",
								Pattern.MULTILINE);
	...
	private void MatchField(String regex, String html, String field,
	...
	public static final String[] urls = new String[] { "ai", "ren", "mei",
			"dao", "de", "zhi", "xin", "zhen", "chengji", "jiushi", "bian",
			"yuedu" };
	public static final String[] kindname = new String[] { "爱", "人", "美", "道",
			"德", "智", "心", "真", "城纪", "旧记忆", "彼岸生活", "悦读" };

	public void loadfromlocal() {
		List<Map<String, Object>> newslist = ndao.getNewsByKindID(inititem,
				skip, currentKindID);

		if (newslist.size() < inititem && next > 0) {
			loadfromnetwork();
		} else {
			fillResult(newslist, next, iscache);
			setNext(next);
			loading = false;
		}
	}
	...
	public void loadfromnetwork() {
		String url = "http://soul.cn.yahoo.com/" + urls[currentKindID]
	...
	public abstract void fillResult(List<Map<String, Object>> list, int next,
			Boolean cache);
}
