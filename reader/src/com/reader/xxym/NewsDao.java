...
//	}
	...
	public long updateNews(int id,String content)
{
open();
ContentValues values = new ContentValues();
values.put("content", content);
long r = database.update("news", values, "_id=?",  new String[]{id+""});
close();
return r;
}
		...
		public long addAllNews(List<Map<String, Object>> list,int kindid)
		{
			...
			try
			{
				for(int i =0;i<list.size();i++)
				{
					...
					{
						ContentValues values = new ContentValues();
						String[] keys = map.keySet().toArray(new String[values.size()]);
	...
	{
		open();
		Cursor c = database.query("news", new String[]{"content","url"},"_id=?", new String[]{id+""}, null, 
				null, "_id desc");
	...
	public List<Map<String, Object>> getNewsByKindID(int limit,int skip,Integer kindid)
	{
		...
		News.TABLE_NAME+" Where "+selection.toString()+" Order by _id asc"+
	...
	{
		open();
		database.delete(News.TABLE_NAME, News.KINDID+"=? ", new String[]{kind+""});
		close();
	}
	...
	{
		open();
		Cursor c = database.rawQuery("select count(*) from "+News.TABLE_NAME+" where "+News.PARENTKINDID+"=?",
	...
	public void deleteNews(int newsid) {
		open();
		database.delete(News.TABLE_NAME, News.NEWSID+"=?", new String[]{newsid+""});
		close();
	}
